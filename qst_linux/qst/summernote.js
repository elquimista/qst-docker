  ...
  */
          ...
          return createImage(src, param).then(function ($image) {
              ...
              if (typeof param === 'function') {
                  ...
                  if (typeof param === 'string') {
					  // added for QST - for choosing an image from QST Files
					  $image.attr('alt', param);

     //                 $image.attr('data-filename', param);
                  	  }
                  $image.css('width', Math.min(_this.$editable.width(), $image.width()));  // sets width in image style
       ...
       * @filedescript added for QST
       */
      Editor.prototype.insertImageQST = function (src, param) {
          var _this = this;
          return createImage(src, param).then(function ($image) {
              _this.beforeCommand();
              if (typeof param === 'function') {
  ...
  var ImageDialog = /** @class */ (function () {
      ...
      ImageDialog.prototype.initialize = function () {
          var $container = this.options.dialogsInBody ? this.$body : this.$editor;
          var body = [     //Added for QST
          '<script>function get_files() {question_image_handler("question"); }</script><B data-dismiss="modal"  data-toggle="tooltip" title="Select a file" style="cursor:pointer" onClick="get_files()">QST Files<BR><BR></B>',
            '<div class="form-group note-group-image-url" style="overflow:auto;">',
              '<label class="note-form-label">' + this.lang.image.url + '</label>',
              '<input class="note-image-url form-control note-form-control note-input ',
              ' col-md-8" type="text" />',

 				// works             '<label class="note-form-label">' + 'Description for image url' + '</label>',

              '<BR>&nbsp;&nbsp;Description for Image URL<P>',
              ' <textarea placeholder="To conform to WCAG 2.0 guidelines, all files must have a description for screen readers.\n 500 characters max." class="note-image-filedescrip form-control note-form-control note-input ',
              ' col-md-8" />',

      		  //       '&nbsp;&nbsp;<textarea name="note-image-filedescrip" id="filedescrip" cols="80" rows="3" placeholder="To conform to WCAG 2.0 guidelines, all files must have a description for screen readers. 500 characters max."></textarea>',
              '</div>',
          ].join('');
	...
	ImageDialog.prototype.destroy = function () {
      ...
      ImageDialog.prototype.show = function () {
          ...
          this.showImageDialog().then(function (data) {
			  ...
			  // Added for QST
			  var $imageFiledescrip = _this.$dialog.find('.note-image-filedescrip');
			  var $whichway = 1;
			  // $imageFiledescrip.val() turns " to &quot;

			  // need to remove new lines and spaces
			  var $str = $imageFiledescrip.val();
			  $str = $str.replace(/[\n\r]/g,'');  // remove new lines
			  $str = $str.replace(/\s/g,'');  // remove spaces

			  // check if empty
				//if ($imageFiledescrip.val() == null || $imageFiledescrip.val() == "") {
				if ($str == null || $str == "") {
					$whichway = 2;
					}
              ...
              if (typeof data === 'string') { // image url
                  ...
                  if (_this.options.callbacks.onImageLinkInsert) {
                      _this.context.triggerEvent('image.link.insert', data);
                        ...
                        //   _this.context.invoke('editor.insertImage', data);            //  original
        				// Added for QST
        				if($whichway == 2) {
							 _this.context.invoke('editor.insertImageQST', data,'No description was added for this file.');   // created editor.insertImageQST for this
							}
						else {
                       		 _this.context.invoke('editor.insertImageQST', data,$imageFiledescrip.val());   // created editor.insertImageQST for this
						 	}
                  		}
              		}
              else { // array of files
      ...
      ImageDialog.prototype.showImageDialog = function () {
          var _this = this;
          return $$1.Deferred(function (deferred) {
			  //Added for QST $imageFiledescrip
//			  var $imageUrl = _this.$dialog.find('.filedescrip');
